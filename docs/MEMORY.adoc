...
Face profiles have a separate Settings panel and are not linked to voice identities. Local voice turns can enrol speakers; background Unmute sampling matches existing profiles when the recognition backend overlay is installed.
