...
class JournalTests(unittest.TestCase):
 def test_local_day_query_and_untrusted_reference(self):
  with tempfile.TemporaryDirectory() as d:
   p=Path(d)/'journal.json';p.write_text(json.dumps({'version':1,'retentionDays':30,'events':[{'at':'2026-09-23T23:30:00Z','source':'vision','kind':'appeared','subject':'grey cat','uncertain':True}]}))
