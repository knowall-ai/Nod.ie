...
class RecognitionSession {
 constructor(renderer){this.renderer=renderer;this.generation=0;this.words=[];this.turns=[];this.askedFaces=new Map();this.logged=new Set();this.createPrompt();}
 ...
 stop(){++this.generation;clearInterval(this.poll);clearTimeout(this.attemptTimer);this.enabled=false;this.words=[];this.turns=[];this.logged=new Set();this.speaking=false;this.clearPrompt();this.send({speaker_tracking_enabled:false,speaker_observation:null,face_observation:null,recognition_feedback:null});window.nodie?.cancelRecognition?.().catch(()=>{});window.nodie?.cancelSpeakers?.().catch(()=>{});}
 clearPrompt(){clearTimeout(this.expiry);this.pending=null;if(this.prompt)this.prompt.hidden=true;}
 event(data){
  ...
  if(data.type==='conversation.item.input_audio_transcription.delta'&&typeof data.delta==='string'&&Number.isFinite(data.start_time)){this.words.push({text:data.delta,start:data.start_time});this.words=this.words.slice(-100);}
  if(data.type==='response.created'){if(this.words.length)this.turns.push({words:this.words,at:Date.now()});this.turns=this.turns.slice(-3);this.words=[];this.speaking=true;}
 ...
 faces(result){const faces=result?.state==='ready'?result.faces:[];this.send({face_observation:faces.map(f=>{const mayAskName=!f.name&&!f.uncertain&&f.id&&Date.now()-(this.askedFaces.get(f.id)||0)>300000;if(mayAskName)this.askedFaces.set(f.id,Date.now());return {name:f.name||null,uncertain:true,mayAskName:Boolean(mayAskName)};})});}
 async propose(){if(this.proposing||this.pending||this.speaking)return;const turn=this.turns.find(t=>!t.attempted&&Date.now()-t.at<15000&&t.words.every(w=>Number.isFinite(w.end)));if(!turn)return;turn.attempted=true;const generation=this.generation;this.proposing=true;try{const result=await window.nodie.proposeRecognitionName({text:turn.words.map(w=>w.text).join(' ').slice(0,2000),start:turn.words[0].start,end:turn.words.at(-1).end});if(generation!==this.generation||result.status!=='pending')return;this.showPrompt(result);}catch{}finally{this.proposing=false;}}
 showPrompt(result){this.pending=result;clearTimeout(this.expiry);this.expiry=setTimeout(()=>{if(this.pending===result)void this.confirm(false);},60000);this.label.textContent=`Use “${result.name}” for this ${result.kind==='face'?'face':'voice'} profile? Confirm here to save.`;this.prompt.hidden=false;this.send({recognition_feedback:{status:'pending',kind:result.kind,name:result.name}});}
 async confirm(accepted){const pending=this.pending;if(!pending)return;this.clearPrompt();try{const result=await window.nodie.confirmRecognitionName(pending.token,accepted);this.send({recognition_feedback:{status:result.status,kind:pending.kind,name:pending.name}});this.renderer.debugStream?.add('Naming',`${pending.kind}: ${pending.name} — ${result.status}`);}catch{this.renderer.showNotification('Profile name could not be saved.','error');}}
 createPrompt(){this.prompt=document.createElement('div');this.prompt.id='recognition-confirm';this.prompt.hidden=true;this.prompt.setAttribute('role','status');this.label=document.createElement('span');this.prompt.append(this.label);for(const [text,value] of [['Confirm',true],['Cancel',false]]){const b=document.createElement('button');b.type='button';b.textContent=text;b.onclick=()=>void this.confirm(value);this.prompt.append(b);}document.body.append(this.prompt);}
}
...
if(typeof window!=='undefined')window.addEventListener('DOMContentLoaded',()=>{if(window.NodieRenderer&&window.nodie?.proposeRecognitionName){const r=window.NodieRenderer;r.recognition ||= new RecognitionSession(r);window.nodie.onRecognitionProposal?.(p=>r.recognition.showPrompt(p));}});
