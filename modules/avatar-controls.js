...
class AvatarControls {
    constructor(renderer) {
        ...
        try { renderer.state.speakerMuted = localStorage.getItem('nodie.speaker-muted') === 'true'; } catch { renderer.state.speakerMuted = false; }
        ...
        this.speaker?.addEventListener('click', () => {
            ...
            try { localStorage.setItem('nodie.speaker-muted', String(renderer.state.speakerMuted)); } catch { /* Still works without persistence. */ }
            this.update();
        });
    ...
    update() {
        ...
        if (this.mic) {
            ...
            this.mic.title = local?.state === 'starting' ? 'Opening microphone — click to cancel' : micOn ? (local ? 'Microphone on — click to send recording' : 'Turn microphone off') : 'Microphone off — click to speak';
        ...
        if (this.speaker) {
            ...
            this.speaker.title = on ? 'Speaker on — mute voice' : 'Speaker off — enable voice';
