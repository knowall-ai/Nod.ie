...
scene_patches = [
    ...
    ('        openai_tools = self.mcp_manager.get_tools_for_openai_api() if self.mcp_manager else []',
     '        scene_tools_blocked = self._scene_tools_blocked\n        openai_tools = self.mcp_manager.get_tools_for_openai_api() if self.mcp_manager and not scene_tools_blocked else []\n        openai_tools = [tool for tool in openai_tools if tool.get("function", {}).get("name") not in ("reverie.resolve_people", "reverie.recall_animals", "reverie.resolve_linked_people")]'),
    ('        messages = self.chatbot.preprocessed_messages()',
     '        messages = self.chatbot.preprocessed_messages()\n        from unmute.person_recall import person_messages\n        messages = await person_messages(messages, self.mcp_manager, self.chatbot.chat_history, include_animals=bool(self._scene_data and self._scene_data.get("status") == "snapshot"), recognition_state=getattr(self,"_nodie_windows",None), faces=getattr(self,"_nodie_faces",None))\n        from unmute.journal_context import journal_messages\n        messages = await asyncio.to_thread(journal_messages, messages, self.chatbot.chat_history)\n        from unmute.node_context import node_messages\n        messages = await asyncio.to_thread(node_messages, messages, self.chatbot.chat_history)\n        if self._scene_data is not None:\n            from unmute.scene_context import scene_messages\n            messages = scene_messages(messages, self._scene_data)'),
