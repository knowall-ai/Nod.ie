...
function resolveTimezone(value,onWarning=message=>console.warn(message)){try{new Intl.DateTimeFormat('en',{timeZone:value});return value;}catch{onWarning('Invalid NODIE_TIMEZONE; using the system timezone for the event journal.');return Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';}}
class EventJournal extends ConversationHistory {
 constructor(file,{now=()=>Date.now(),timezone=process.env.NODIE_TIMEZONE||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC',onWarning}={}){super(file);this.now=now;this.timezone=resolveTimezone(timezone,onWarning);}
