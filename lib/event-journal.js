...
class EventJournal extends ConversationHistory {
 ...
 empty(){return {version:1,epoch:crypto.randomUUID(),retentionDays:30,enabled:true,events:[]};}
 ...
 load(){return this.transaction(async()=>{const d=await this.read(),count=d.events.length;this.prune(d);if(this.missing||count!==d.events.length)await this.write(d);return d;});}
 ...
 async reference(text){
  ...
  const now=this.now(),day=new Date(now-(/yesterday/i.test(text)?86400000:0));const key=date=>new Intl.DateTimeFormat('en-CA',{timeZone:process.env.NODIE_TIMEZONE||'Europe/London',year:'numeric',month:'2-digit',day:'2-digit'}).format(date);
  ...
  try{const data=await this.load(),events=data.events.filter(e=>{const date=key(new Date(e.at));return date>=from&&date<=through;});return {status:'ok',from,through,timezone:process.env.NODIE_TIMEZONE||'Europe/London',total:events.length,truncated:events.length>60,events:events.slice(-60)};}catch{return {status:'unavailable'};}
 }
