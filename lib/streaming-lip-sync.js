...
class StreamingLipSync {
    ...
    async render(audio, generation = this.generation, trim) {
        ...
        if(trim!==undefined && (!trim || typeof trim!=='object' || Array.isArray(trim) || Object.keys(trim).length!==2 || !Object.hasOwn(trim,'startFrame') || !Object.hasOwn(trim,'frameCount') || !Number.isInteger(trim.startFrame) || trim.startFrame<0 || trim.startFrame>8 || !Number.isInteger(trim.frameCount) || trim.frameCount<1 || trim.frameCount>33 || (trim.startFrame+trim.frameCount)/25>size/(rate*2)+1e-6)) throw new Error('Invalid lip-sync frame range');
        const controller = this.current = new AbortController();
        try { return await renderSpeech(audio, { url: this.url, videoOnly: true, trim, signal: AbortSignal.any([controller.signal, AbortSignal.timeout(2500)]) }); }
