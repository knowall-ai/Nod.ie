...
class StreamingLipSync {
    constructor({ url, enabled = () => true } = {}) { this.url = url; this.enabled = enabled; this.current = null; }
    async render(audio) {
        ...
        try { return await renderSpeech(audio, { url: this.url, signal: AbortSignal.any([controller.signal, AbortSignal.timeout(2500)]) }); }
