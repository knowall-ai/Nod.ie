(function (root) {
    const aliases = { CURIOSITY_LEVEL: 'curiosityLevel', UNMUTE_BACKEND_URL: 'unmuteBackendUrl', VOICE_MODEL: 'voiceModel', LLM_MODEL: 'llmModel', ASSISTANT_NAME: 'assistantName', GLOBAL_HOTKEY: 'globalHotkey', AVATAR_ENABLED: 'avatarEnabled', AVATAR_IDLE_ENABLED: 'avatarIdleEnabled' };
    const publicKeys = ['CURIOSITY_LEVEL','UNMUTE_BACKEND_URL', 'VOICE_MODEL', 'LLM_MODEL', 'ASSISTANT_NAME', 'GLOBAL_HOTKEY', 'AVATAR_ENABLED', 'AVATAR_IDLE_ENABLED', 'MUSETALK_HTTP', 'MUSETALK_WS', 'WEB_TEST_PORT', 'VOICE_MODE', 'LIP_SYNC_CONFIGURED'];
    function normalize(input = {}) {
        ...
        result.CURIOSITY_LEVEL = result.CURIOSITY_LEVEL ?? 'off';
        return result;
    }
    ...
    function validate(input) {
        ...
        if (!['off','observe','low','normal','high'].includes(config.CURIOSITY_LEVEL)) throw new Error('Invalid curiosity level');
