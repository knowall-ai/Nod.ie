        ...
        try {
            ...
            if (req.url.startsWith('/voice/')) {
                ...
                try {
                    ...
                    return res.end(JSON.stringify({ ...result, audio: Buffer.from(result.audio).toString('base64') }));
                } catch { res.writeHead(502); return res.end(JSON.stringify({ error: 'Voice request failed. Check local service availability.' })); }
            }
...
function start() {
    ...
    const server = createServer({ config, monitor });
    monitor.start().catch(() => {});
