...
const files = new Set(['index.html', 'browser-bridge.js', 'renderer.js', 'renderer.css', 'config-web.js', 'lib/config-schema.js', 'modules/curiosity-session.js', 'modules/recognition-session.js', 'modules/debug-stream.js', 'modules/window-controls.js', 'modules/spoken-controls.js', 'modules/streaming-transcript.js', 'modules/streaming-lip-sync.js', 'modules/audio-capture-web.js', 'modules/audio-playback-web.js', 'modules/avatar-manager.js', 'modules/idle-avatar.js', 'modules/musetalk-websocket-client.js', 'modules/websocket-handler.js', 'modules/local-voice-session.js', 'modules/end-of-speech.js', 'modules/desktop-drag.js', 'modules/overlay-hit-regions.js', 'modules/stream-speakers.js', 'modules/vision-frame-selector.js', 'modules/vision-camera.js', 'modules/vision-context.js', 'modules/avatar-controls.js', 'assets/icons/lucide/settings.svg', 'assets/icons/lucide/mic.svg', 'assets/icons/lucide/mic-off.svg', 'assets/icons/lucide/camera-off.svg', 'assets/icons/lucide/volume-2.svg', 'assets/icons/lucide/volume-x.svg', 'audio-output-processor.js', 'decoderWorker.min.js', 'decoderWorker.min.wasm', 'encoderWorker.min.js', 'node_modules/opus-recorder/dist/recorder.min.js', 'tests/test-web.html', 'favicon.ico']);
function createServer({ root = path.resolve(__dirname, '..'), config = require('../config'), voice, monitor, diagnostics, logger, historyStore = new (require('./conversation-history').ConversationHistory)(path.join(require('node:os').homedir(), '.config/nodie/conversations/local.json')) } = {}) {
    ...
    const vision = new (require('./vision-analysis').VisionAnalysis)({ url: config.getConfig?.('OLLAMA_URL') || 'http://127.0.0.1:11434', model: config.getConfig?.('LOCAL_VISION_MODEL') || config.LLM_MODEL || 'nodie-qwen3.5:9b' });
    return http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (req.url.startsWith('/vision/')) {
                if (req.method !== 'POST') return fail(405);
                if (req.headers.origin !== `http://${req.headers.host}` || (req.headers['sec-fetch-site'] && req.headers['sec-fetch-site'] !== 'same-origin')) return fail(403);
                res.setHeader('Content-Type', 'application/json');
                if (req.url === '/vision/cancel') { vision.cancel(); return res.end('{}'); }
                if (req.url !== '/vision/analyse') return fail(404);
                const generation = vision.generation;
                const chunks = []; let size = 0;
                const deadline = setTimeout(() => req.destroy(), 3000);
                req.once('close', () => clearTimeout(deadline));
                try {
                    for await (const chunk of req) { size += chunk.length; if (size > 512000) return fail(413); chunks.push(chunk); }
                    clearTimeout(deadline);
                    if (generation !== vision.generation) return res.end(JSON.stringify({ status: 'cancelled' }));
                    return res.end(JSON.stringify(await vision.analyse(new Uint8Array(Buffer.concat(chunks)))));
                } finally { clearTimeout(deadline); }
            }
    ...
    }).on('close', () => { streamLips.cancel(); vision.cancel(); });
}
function start() {
    ...
    const voice = config.VOICE_MODE === 'local' ? new (require('./local-voice').LocalVoice)({ logger, historyStore, speakerRecognition: new (require('./speaker-recognition').SpeakerRecognition)(), diagnostics: () => diagnostics.status() }) : undefined;
    const server = createServer({ config, voice, monitor, diagnostics, logger, historyStore });
    diagnostics.start();
