...
const { nodeTool } = require('./node-snapshot');
...
class LocalVoice {
    ...
    async converse(audio) {
        ...
        try {
            ...
            let result = await chat([nodeTool]);
            if (result.message?.tool_calls?.length) {
                const calls = result.message.tool_calls;
                if (calls.length !== 1 || calls[0].function?.name !== 'get_node_status') throw new Error('Unsupported tool call');
                const args = calls[0].function.arguments;
                if (!args || typeof args !== 'object' || Array.isArray(args) || Object.keys(args).length) throw new Error('Invalid tool arguments');
                const snapshot = await this.nodeSnapshot({ signal: this.abortController.signal });
                messages.push({ role: 'assistant', content: result.message.content || '', tool_calls: calls }, { role: 'tool', tool_name: 'get_node_status', content: JSON.stringify(snapshot) });
                // One read-only tool round per turn; no recursive calls or shell access.
                result = await chat();
