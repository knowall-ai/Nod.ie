...
class LocalVoice {
    ...
    async converse(audio) {
        ...
        try {
            ...
            const chat = async tools => this.timed('model', async () => (await this.request(this.endpoint('OLLAMA_URL') + '/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: this.config('LOCAL_LLM_MODEL'), ...(/^qwen3\.5(?=:|$)/i.test(this.config('LOCAL_LLM_MODEL') || '') ? { think: false } : {}), messages, ...(tools ? { tools } : {}), stream: false, options: { num_predict: 160, num_ctx: 8192 } }) })).json());
