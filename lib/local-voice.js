...
class LocalVoice {
    constructor({ fetchImpl = fetch } = {}) { this.fetch = fetchImpl; this.history = []; this.busy = false; this.memoryState = 'not checked'; }
    endpoint(key) {
    ...
    async health() {
        ...
        const model = getConfig('LOCAL_LLM_MODEL');
    ...
    async connectMemory() {
        const file = getConfig('REVERIE_CONFIG_PATH');
        const name = getConfig('REVERIE_CONFIG_SERVER');
    ...
    async converse(audio) {
        ...
        try {
            const form = new FormData(); form.append('file', new Blob([audio]), 'speech.webm'); form.append('model', getConfig('LOCAL_STT_MODEL', 'Systran/faster-whisper-tiny.en')); form.append('language', 'en');
            ...
            const result = await (await this.request(this.endpoint('OLLAMA_URL') + '/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: getConfig('LOCAL_LLM_MODEL'), messages, stream: false, options: { num_predict: 90, num_ctx: 8192 } }) })).json();
            const reply = result.message?.content?.trim(); if (!reply) throw new Error('The language model returned no reply');
            const speech = await this.request(this.endpoint('LOCAL_TTS_URL') + '/api/tts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: reply, language: 'en', speaker: getConfig('LOCAL_TTS_VOICE', 'af_heart'), speed: Number(getConfig('LOCAL_TTS_SPEED', '1.06')) }) });
