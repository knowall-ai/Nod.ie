...
class LocalVoice {
    ...
    async health() {
        ...
        return { ready: unavailable.length === 0 && Boolean(model), unavailable, model, modelLoaded: loaded, memory: this.memoryState, mode: 'local', avatar: { enabled: this.avatarEnabled(), lipSyncConfigured: Boolean(this.config('LOCAL_LIP_SYNC_URL')) }, note: 'Click to record, then click again to send. Transcription may load a small model on the first turn.' };
    }
    async memory() {
