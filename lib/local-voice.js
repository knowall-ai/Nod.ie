...
const { isAddressedToNodie } = require('./direct-address');
...
const controlsTool = { type: 'function', function: { name: 'set_voice_controls', description: 'Only for an explicit request to change the microphone or speaker, never for a camera request. Include ONLY the device the user asked to change; omit other device fields entirely. For stop listening send microphoneEnabled=false only. For mute yourself send speakerEnabled=false only. For unmute send speakerEnabled=true only. This cannot start a microphone or control a camera. Compose your own brief acknowledgement.', parameters: { type: 'object', properties: { microphoneEnabled: { type: 'boolean', enum: [false] }, speakerEnabled: { type: 'boolean' }, reply: { type: 'string', description: 'Your brief final acknowledgement. A speaker-mute command is applied silently.' } }, required: ['reply'], additionalProperties: false } } };
class LocalVoice {
    ...
    async converse(audio) {
        ...
        try {
            ...
            const addressed = isAddressedToNodie(text);
            const deviceTools = addressed ? [controlsTool] : [];
            ...
            if (mandatorySnapshot) {
                ...
                result = await chat([replyTool, ...deviceTools]);
            } else result = await chat([nodeTool, replyTool, ...deviceTools]);
            if (result?.message?.tool_calls?.length) {
                ...
                if (fn.name === 'respond_to_user') {
                    ...
                    if (!addressed) throw new Error('Device controls require direct address');
                    ...
                    controls = { ...(Object.hasOwn(args, 'microphoneEnabled') ? { microphoneEnabled: false } : {}), ...(Object.hasOwn(args, 'speakerEnabled') ? { speakerEnabled: args.speakerEnabled } : {}) };
                    reply = args.reply.trim();
