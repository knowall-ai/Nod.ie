...
class LocalVoice {
    constructor({ fetchImpl = fetch, config = getConfig, logger, diagnostics = () => ({ state: 'not-configured' }) } = {}) { this.logger = logger; this.diagnostics = diagnostics; this.config = config; this.fetch = fetchImpl; this.history = []; this.busy = false; this.memoryState = 'not checked'; }
    endpoint(key) {
    ...
    async converse(audio) {
        ...
        let stage = 'transcription'; const started = Date.now();
        try {
