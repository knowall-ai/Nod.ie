        ...
        try {
            ...
            if (result?.message?.tool_calls?.length) {
                ...
                if (fn.name === 'respond_to_user') {
                    ...
                    const applied = await this.speakerRecognition.name(speakerObservation, args.id, args.name, this.abortController.signal);
                    messages.push({ role: 'assistant', content: '', tool_calls: calls }, { role: 'tool', tool_name: fn.name, content: JSON.stringify({ applied }) });
