...
class LocalVoice {
    constructor({ fetchImpl = fetch, config = getConfig, logger, historyStore, avatarEnabled = () => ![false, 'false'].includes(config('AVATAR_ENABLED')), diagnostics = () => ({ state: 'not-configured' }) } = {}) { this.avatarEnabled = avatarEnabled; this.historyStore = historyStore; this.logger = logger; this.diagnostics = diagnostics; this.config = config; this.fetch = fetchImpl; this.history = []; this.busy = false; this.memoryState = 'not checked'; }
    endpoint(key) {
