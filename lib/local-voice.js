...
class LocalVoice {
    constructor({ fetchImpl = fetch, config = getConfig, logger, historyStore, speakerRecognition, controlIntent, nodeSnapshot = require('./node-snapshot').nodeSnapshot, avatarEnabled = () => ![false, 'false'].includes(config('AVATAR_ENABLED')), diagnostics = () => ({ state: 'not-configured' }) } = {}) { this.speakerRecognition = speakerRecognition; this.controlIntent = controlIntent || this.checkControlIntent.bind(this); this.nodeSnapshot = nodeSnapshot; this.avatarEnabled = avatarEnabled; this.historyStore = historyStore; this.logger = logger; this.diagnostics = diagnostics; this.config = config; this.fetch = fetchImpl; this.history = []; this.busy = false; this.memoryState = 'not checked'; }
    endpoint(key) {
    ...
    async converse(audio) {
        ...
        try {
            ...
            return { transcript: text, reply, ...(speakerObservation ? { speakers: { state: speakerObservation.state, speakers: speakerObservation?.state === 'ready' ? speakerObservation : undefined.speakers, segments: speakerObservation.segments, transcriptAttribution: speakerObservation.transcriptAttribution } } : {}), audio: bytes, video, lipSync, memory: this.memoryState, ...(controls ? { controls } : {}), ...(silent ? { silent: true } : {}) };
        } catch (error) {
