    ...
    observe(result, epoch, signal, {temporary=false}={}) {
        ...
        return this.transaction(async () => {
            ...
            for (const face of result.faces) {
                ...
                else if (d.profiles.length < 32) { p = { id: crypto.randomUUID(), name: null, vectors: [face.vector], lastSeen: Date.now() }; if(temporary){this.candidates.push(p);this.candidates=this.candidates.slice(-16);}else d.profiles.push(p); }
