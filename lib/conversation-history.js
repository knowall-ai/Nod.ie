...
const wordsOK=words=>words===undefined || (Array.isArray(words)&&words.length<=100&&words.every(w=>w&&typeof w.text==='string'&&w.text.length<=200&&Number.isFinite(w.start)&&w.start>=0&&w.start<1e8&&(w.end===undefined||(Number.isFinite(w.end)&&w.end>=w.start&&w.end-w.start<=5))&&(w.speaker===undefined||w.speaker===null||(typeof w.speaker==='string'&&w.speaker.length<=80))&&[undefined,'unattributed','possible-match'].includes(w.attribution)&&Object.keys(w).every(k=>['text','start','end','speaker','attribution'].includes(k))));
class ConversationHistory {
    ...
    async read() {
        ...
        if (!data) return null;
        if (data.version !== 1 || typeof data.epoch !== 'string' || !Array.isArray(data.turns) || data.turns.length > 12 || data.turns.some(t => !['user', 'assistant'].includes(t.role) || typeof t.content !== 'string' || t.content.length > 2000 || !wordsOK(t.words))) throw new Error('Invalid conversation history');
        return data;
    }
    async write(data) {
    ...
    upsert(epoch, turn) {
        if (typeof epoch !== 'string' || epoch.length > 100 || !turn || typeof turn !== 'object' || Array.isArray(turn) ||
            Object.keys(turn).some(key => !['id', 'role', 'content', 'words'].includes(key)) ||
            typeof turn.id !== 'string' || !/^[a-zA-Z0-9-]{1,100}$/.test(turn.id) ||
            !['user', 'assistant'].includes(turn.role) || typeof turn.content !== 'string' ||
            !turn.content.trim() || turn.content.length > 2000 || !wordsOK(turn.words)) throw new Error('Invalid transcript');
        return this.transaction(async () => {
            const data = await this.read();
            ...
            if(previous){previous.content=turn.content;if(turn.words)previous.words=turn.words;}
            else data.turns.push({ id: turn.id, role: turn.role, content: turn.content, ...(turn.words?{words:turn.words}:{}), at: new Date().toISOString() });
