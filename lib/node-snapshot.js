...
const integer = value => { const n = Number(value); if (!Number.isSafeInteger(n) || n < 0) throw new Error('Invalid node metric'); return n; };
...
async function nodeSnapshot({ run = runFile, signal } = {}) {
    ...
    const [bitcoin, lightning] = await Promise.allSettled([
        ...
        Promise.all([lnd('getinfo'), lnd('listchannels')]).then(([info, result]) => {
            ...
            return { state: 'checked', implementation: 'LND', version: version(info.version), openChannels: channels.length, activeChannels: channels.filter(c => c.active).length, pendingChannels: integer(info.num_pending_channels), totals: totals(channels), activeTotals: totals(channels.filter(c => c.active)), channels: channels.slice(0, 20), channelDetailsTruncated: channels.length > 20, note: 'All values are sats. Local/remote balances describe outbound/inbound balance, not guaranteed routable liquidity. Reserves, fees, HTLC limits and peer availability restrict payments. Inactive channels are excluded from activeTotals. Channel calls are a best-effort snapshot, not atomic.' };
        })
    ]);
...
module.exports = { nodeSnapshot, nodeTool };
