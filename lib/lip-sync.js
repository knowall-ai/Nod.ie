...
async function renderSpeech(audio, { url, signal, videoOnly = false, fetchImpl = fetch }) {
    ...
    const response = await fetchImpl(endpoint.href.replace(/\/$/, '') + '/render', {
        method: 'POST', headers: { 'Content-Type': 'audio/wav', ...(videoOnly ? { 'X-Nodie-Video-Only': '1' } : {}) }, body: audio,
