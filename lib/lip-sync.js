...
async function renderSpeech(audio, options) {
    if (!options || typeof options !== 'object' || Array.isArray(options) || Object.keys(options).some(key => !['url', 'signal', 'videoOnly', 'fetchImpl'].includes(key))) throw new Error('Invalid lip-sync options');
    const { url, signal, videoOnly = false, fetchImpl = fetch } = options;
    ...
    const response = await fetchImpl(endpoint.href.replace(/\/$/, '') + '/render', {
        method: 'POST', headers: { 'Content-Type': 'audio/wav', ...(videoOnly ? { [VIDEO_ONLY_HEADER]: VIDEO_ONLY_VALUE } : {}) }, body: audio,
