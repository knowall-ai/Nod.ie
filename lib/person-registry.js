...
class PersonRegistry extends ConversationHistory {
 ...
 save({id,name:label,faceId,voiceId,memory},profiles){
  if(id!==null&&!uuid(id)||!name(label)||faceId!==null&&!uuid(faceId)||voiceId!==null&&!uuid(voiceId)||!faceId&&!voiceId)throw Error('Choose at least one named recognition profile');
