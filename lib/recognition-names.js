...
const nameOK = name => typeof name === 'string' && name.trim().length > 0 && name.length <= 80 && !/[\p{Cc}\p{Cf}]/u.test(name);
class RecognitionNames {
    ...
    cancel() { ++this.generation; this.pending = null; this.controller?.abort(); }
    async propose(turn) {
        if (!turn || typeof turn.text !== 'string' || !turn.text.trim() || turn.text.length > 2000 || !Number.isFinite(turn.start) || !Number.isFinite(turn.end) || turn.start < 0 || turn.end <= turn.start || turn.end - turn.start > 30) throw Error('Invalid introduction');
        if (this.pending || this.busy) return { status: 'not-saved' };
        ...
        if (!voice && !freshFace) return { status: 'not-saved' };
        this.busy = true;this.controller=new AbortController();
        try {
            const intent = await this.classify(turn.text,this.controller.signal);
    ...
    async proposeLocal(observation, text) {
        ...
        try {
            const intent=await this.classify(text,this.controller.signal);
