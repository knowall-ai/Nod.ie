    ...
    async observe(result, epoch, signal) {
        ...
        return this.transaction(async () => {
            ...
            for (const s of result.speakers) {
                ...
                else if (d.profiles.length < 32 && d.profiles.reduce((n, p) => n + samples(p).length, 0) < 64) { p = { id: crypto.randomUUID(), model: MODEL, vector: s.embedding, name: null, lastSeen: Date.now(), lastAsked: 0 }; d.profiles.push(p); }
