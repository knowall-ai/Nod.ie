    ...
    async observe(result, epoch, signal, { enrol = true } = {}) {
        ...
        return this.transaction(async () => {
            ...
            for (const s of result.speakers) {
                ...
                else if (enrol && d.profiles.length < 32 && d.profiles.reduce((n, p) => n + samples(p).length, 0) < 64) { p = { id: crypto.randomUUID(), model: MODEL, vector: s.embedding, name: null, lastSeen: Date.now(), lastAsked: 0 }; d.profiles.push(p);dirty=true; }
                if (!p) { speakers.push({ speaker: s.speaker, uncertain: true }); continue; }
                used.add(p.id);if(Date.now()-p.lastSeen>=60000){p.lastSeen=Date.now();dirty=true;}
                const mayAskName = enrol && !p.name && Date.now() - p.lastAsked > 300000;
                if (mayAskName) {p.lastAsked=Date.now();dirty=true;}
