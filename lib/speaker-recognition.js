...
class SpeakerStore extends ConversationHistory {
    ...
    async status() { return this.transaction(async () => { const d = await this.read(); return { enabled: d.enabled, profiles: d.profiles.map(({ id, name, lastSeen }) => ({ id, name, lastSeen })) }; }); }
    ...
    async observe(result, epoch, signal) {
        ...
        return this.transaction(async () => {
            ...
            for (const s of result.speakers) {
                ...
                const ranked = d.profiles.map(p => ({ p, score: cosine(p.vector, s.embedding) })).sort((a, b) => b.score - a.score);
                let p;
                ...
                else if (d.profiles.length < 32) { p = { id: crypto.randomUUID(), model: MODEL, vector: s.embedding, name: null, lastSeen: Date.now(), lastAsked: 0 }; d.profiles.push(p); }
...
class SpeakerRecognition {
    ...
    async analyse(audio, signal) {
        try {
            ...
            return await this.store.observe(JSON.parse(text), saved.epoch, signal);
