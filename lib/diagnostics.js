...
function changes(previous, current) {
    ...
    for (const item of current) {
        ...
        if (item.health === 'unhealthy') add('unhealthy', 'Container health check is failing.');
...
class Diagnostics {
    ...
    async sample() {
        try {
            ...
            const format = '{"id":{{json .Id}},"name":{{json .Name}},"state":{{json .State.Status}},"health":{{if .State.Health}}{{json .State.Health.Status}}{{else}}null{{end}},"restarts":{{json .RestartCount}},"oom":{{json .State.OOMKilled}}}';
